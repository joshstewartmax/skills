...
3. Pick the runner count. Spawn N candidates on your default model. Diversity here comes from parallel runs of one model, so lean toward more candidates when the arena covers multiple design directions. Two or three is enough when the work is generation-bound rather than judgment-sensitive.
