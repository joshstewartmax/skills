...
After the explanation is complete, spawn several architectural critics in parallel on your default model, all in a single message. Two or three independent runs surface more than one does.
...
You're a pragmatic lead here, not an aggregator. Weigh the critics against the full context (the goal, the constraints, what's already been considered) and sort their findings into what to act on, what to consider, and what to dismiss with a reason.
