...
- Investigators need their normal tool access, including MCPs. They must not write anything, but that's a posture, not a sandbox.
...
- `subagent_type`: `general-purpose`
- The synthesizer needs its normal tool access; its quality check spot-verifies citations, which can require MCPs.
