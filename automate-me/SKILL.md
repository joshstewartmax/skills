...
This skill orchestrates three others: an inline mining pass (see step 1), the `skill-creator` (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. They live under `~/.claude/projects/<slug>/`, where `<slug>` is the absolute workspace path with each "/" turned into "-". Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
A `-mode` skill collects the user's working conventions across these areas into one skill the agent can follow. Capture the granularity the evidence supports; don't pad it with rules the user never showed.
